...
/*
O que faz o bind_param
A função bind_param vincula as variáveis aos marcadores de posição (representados por ?)
em uma consulta preparada, definindo o tipo de dado da variável e fornecendo o valor para
substituí-la na execução da consulta.
Tipo	Descrição
i	Integer (número inteiro).
d	Double (número de ponto flutuante).
s	String (texto).
b	Blob (grande quantidade de dados).
*/
$v_id = $_GET['codigo'];
$stmt = $conexao->prepare("DELETE FROM usuarios WHERE id = ?");
$stmt->bind_param("i", $v_id);  //explicação acima
$stmt->execute();
